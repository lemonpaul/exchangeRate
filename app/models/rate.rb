...
class Rate < ApplicationRecord
  ...
  def self.today
    ...
    Rate.cached_all.select { |rate| rate.created_at.to_date == Time.now.to_date }
  end

  def self.find(currency, operation)
    rates = Rate.today.select do |rate|
  ...
  def self.counts
    {usd: {buy: Rate.find(USD, BUY).count,
           sell: Rate.find(USD, SELL).count},
     eur: {buy: Rate.find(EUR, BUY).count,
           sell: Rate.find(BUY, SELL).count}}
  end
